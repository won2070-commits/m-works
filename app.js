...
/* ═══════════════════ 브랜드 ═══════════════════ */
const APP_VERSION = 'v31 · 2026-07-21';
...
function openManual() {
  modal('사용 설명서 — M.Works ' + APP_VERSION, `
    ...
    <p>설정 → <b>🧩 프롬프트·세부 프롬프트</b>에 나오는 것들의 관계입니다.</p>
    <p><b>① 프롬프트</b> — 각 단계별 작업에 필요한 <b>기본 지시문</b>입니다. 본문 추천·중심사상 분석·설교문 작성·FCF 찾기 등 AI 기능 하나마다 하나씩 있으며, 『들리는 설교, 끌리는 설교』의 방법론이 여기에 구현되어 있습니다. 하나의 프롬프트는 아래 ③+④ 두 부분으로 구성됩니다.</p>
    <p><b>② 세부 프롬프트</b> — 기본 프롬프트 <b>외에 추가로 얹는 지시문</b>입니다. 각 단계 안에서 더 세부적인 필요를 규정합니다. 예: "본문 추천 — 이런 것들을 참조하라", "중심사상 — 이런 것을 하라". 적용 단계를 지정하면 그 단계의 프롬프트 뒤에 자동으로 결합됩니다. 무엇을 써도 앱이 망가지지 않는 안전한 자리입니다.</p>
    <p><b>③ 시스템 지시</b> (프롬프트의 위 상자) — AI에게 "너는 누구이고 어떤 원칙으로 일하라"를 정하는 부분입니다. 역할·방법론·환각 금지·문체 원칙이 들어 있고, 매번 똑같이 적용됩니다. 비유하면 <b>직원의 근로계약서</b>입니다.</p>
    <p><b>④ 사용자 프롬프트 틀</b> (프롬프트의 아래 상자) — 버튼을 누를 때마다 실제로 보내지는 <b>주문서의 양식</b>입니다. 안의 <code>{{주제}}</code> <code>{{본문}}</code> 같은 빈칸을 앱이 그 프로젝트의 실제 데이터로 채워 보냅니다. 비유하면 <b>매번 건네는 작업 주문서 양식</b>입니다.</p>
    <p><b>권장 사용법</b> — ③④(프롬프트의 두 상자)는 웬만하면 손대지 마세요. 특히 <code>{{빈칸}}</code> 표기와 JSON 형식 부분이 틀어지면 버튼이 작동하지 않을 수 있습니다. 원하는 방향·취향·강조는 <b>② 세부 프롬프트</b>에, 설교문에 반드시 지켜질 사항은 <b>나의 작성 규칙</b>(설정 → 📏)에 쓰는 것이 바람직합니다. 기본 프롬프트를 고칠 때는 문장 표현만, 한 번에 하나씩 바꿔 시험하고, 이상해지면 [기본값 복원]으로 되돌리세요.</p>
    </div>`);
}
