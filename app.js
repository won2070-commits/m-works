...
async function fetchStatus(refresh) {
  ...
  if (aiStatus.serverless) {
    ...
    else if (DB.settings.accessCode) { badge.textContent = '● AI 연결됨 (사용 코드)'; badge.className = 'ai-badge ok'; }
...
function aiConnected() {
  if (aiStatus.serverless) return !!(DB.settings.apiKey || DB.settings.accessCode);
  return aiStatus.envKey || aiStatus.cli || !!DB.settings.apiKey || !!DB.settings.accessCode;
}
...
const SPEED_TIERS = {
  ...
  // 나머지(sermon·central·recommend·partial·weave·formatConvert·formatRecommend·
...
const PROXY_URL = 'https://hansung-tsj.netlify.app/api/mworks-ai';
async function callDirect(body, opts) {
  // 내 API 키가 있으면 Anthropic 직접, 없고 사용 코드가 있으면 중계 서버로
  const useProxy = !DB.settings.apiKey && DB.settings.accessCode;
  const r = await fetch(useProxy ? PROXY_URL : 'https://api.anthropic.com/v1/messages', {
    method: 'POST',
    signal: aiAbort.signal,
    headers: useProxy ? {
      'content-type': 'application/json',
      'x-mworks-code': DB.settings.accessCode,
    } : {
    ...
    body: JSON.stringify(useProxy ? body : {
  ...
  if (!r.ok) {
    ...
    if (useProxy && msg) throw new Error(msg);
...
const APP_VERSION = 'v59 · 2026-07-22';
...
function openManual() {
  modal('사용 설명서 — M.Works ' + APP_VERSION, `
    ...
    <h4>2. AI 연결</h4>
    <p><b>배포받은 분들</b>: 설정의 <b>사용 코드로 연결</b>에 발급받은 코드(MW-XXXX-XXXX)를 넣으면 끝입니다. API 키는 서버에만 있고, 월 사용 한도 안에서 쓸 수 있습니다.</p>
    <p><b>관리자</b>: 설정 → 🛠 배포 관리에서 코드를 발급·중지하고 사용량을 봅니다. 교회 비밀번호를 아는 분은 <b>간편 연결</b>도 됩니다.</p>
...
function openSettings() {
  ...
  const body = modal('설정', `
    ...
    <div class="field" style="margin-top:10px">
      <label>사용 코드로 연결 <span class="opt">(배포받은 분들은 이 방법 — 발급받은 코드를 넣으세요)</span></label>
      <div style="display:flex;gap:8px">
        <input id="set-acode" value="${esc(s.accessCode || '')}" placeholder="MW-XXXX-XXXX" style="flex:1;font-family:var(--mono)" autocomplete="off">
        <button class="btn btn-primary btn-sm" id="set-acode-go">연결</button>
      </div>
      <div class="hint">코드는 관리자(도원욱 목사)가 발급합니다. 월 사용 한도가 있으며, API 키는 서버에만 보관됩니다.</div>
    </div>
    <div class="field" style="margin-top:10px">
    ...
    <div class="btn-row">
      ...
      <button class="btn btn-ghost" id="set-dist">🛠 배포 관리 (관리자)</button>
    </div>
  ...
  body.querySelector('#set-dist').addEventListener('click', async () => {
  ...
  body.querySelector('#set-acode-go').addEventListener('click', async () => {
    const code = $('#set-acode').value.trim().toUpperCase();
    if (!code) { s.accessCode = ''; save(true); fetchStatus(); toast('사용 코드를 지웠습니다.'); return; }
    const btn = body.querySelector('#set-acode-go'); btn.disabled = true; btn.textContent = '확인 중…';
    try {
      const r = await (await fetch(ADMIN_URL, { method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ action: 'check', code }) })).json();
      if (r.ok) {
        s.accessCode = code; save(true); await fetchStatus(true);
        toast('연결되었습니다' + (r.name ? ' — ' + r.name + '님' : '') + '. 이번 달 사용량 ' + (r.usedPct || 0) + '%');
        closeModal();
      } else toast(r.message || '코드를 확인하지 못했습니다.', 5000);
    } catch (e) { toast('연결 실패: ' + e.message, 5000); }
    btn.disabled = false; btn.textContent = '연결';
  });
...
/* ═══════════════════ 배포 관리 — 사용 코드 발급·사용량 (관리자) ═══════════════════ */
const ADMIN_URL = 'https://hansung-tsj.netlify.app/.netlify/functions/mworks-admin';
async function adminCall(hash, payload) {
  ...
  if (!r.ok) throw new Error(j.message || j.error || ('오류 ' + r.status));
  return j;
}
const fmtTok = n => n >= 1000000 ? (n / 1000000).toFixed(1) + 'M' : n >= 1000 ? Math.round(n / 1000) + 'k' : String(n);
async function openDistAdmin(hash) {
  let data;
  try { data = await adminCall(hash, { action: 'list' }); }
  catch (e) { toast(e.message, 5000); return; }
  const body = modal('배포 관리 — 사용 코드', `
    <div class="fb-item" style="background:${data.keyConfigured ? 'var(--mint)' : 'var(--pink)'}">
      <b>서버 API 키</b>${data.keyConfigured
        ? '등록되어 있습니다. 코드만 발급하면 바로 쓸 수 있습니다.'
        : '아직 등록되지 않았습니다 — 코드를 발급해도 AI가 작동하지 않습니다. (Netlify 환경변수 MWORKS_ANTHROPIC_KEY)'}
    </div>
    <div class="btn-row" style="margin-top:12px">
      <button class="btn btn-primary btn-sm" id="da-new">＋ 새 코드 발급</button>
      <span style="font-size:.76rem;opacity:.8">${data.month} 사용량 기준 · 한도는 월 가중 토큰(기본 3M ≈ 설교 40편)</span>
    </div>
    <div id="da-list" style="margin-top:10px">
      ${data.rows.length ? data.rows.map(r => `
        <div class="fb-item" style="background:var(--surface-soft);display:flex;gap:10px;align-items:center;flex-wrap:wrap">
          <div style="flex:1;min-width:180px">
            <b style="font-family:var(--mono)">${esc(r.code)}</b> ${esc(r.name || '')}
            ${r.active ? '' : '<span class="badge" style="background:var(--pink)">중지됨</span>'}
            <div style="font-size:.74rem;opacity:.8;margin-top:3px">
              이번 달 ${fmtTok(r.usedTokens)} / ${fmtTok(r.limitTokens)} (${Math.min(100, Math.round(r.usedTokens / r.limitTokens * 100))}%) · ${r.calls}회
              ${r.lastUsed ? ' · 마지막 ' + new Date(r.lastUsed).toLocaleDateString('ko-KR') : ''}
            </div>
            <div class="mini-progress" style="margin-top:4px"><i style="width:${Math.min(100, Math.round(r.usedTokens / r.limitTokens * 100))}%"></i></div>
          </div>
          <button class="btn btn-ghost btn-sm" data-da-copy="${esc(r.code)}">복사</button>
          <button class="btn btn-ghost btn-sm" data-da-toggle="${esc(r.code)}" data-active="${r.active ? 1 : 0}">${r.active ? '중지' : '재개'}</button>
          <button class="btn btn-ghost btn-sm" data-da-limit="${esc(r.code)}">한도</button>
          <button class="btn btn-danger btn-sm" data-da-del="${esc(r.code)}">삭제</button>
        </div>`).join('') : '<p style="font-size:.86rem;opacity:.75">발급된 코드가 없습니다. [＋ 새 코드 발급]으로 시작하세요.</p>'}
    </div>
    <p class="ai-note" style="margin-top:12px">코드를 받은 사람은 <b>설정 → 사용 코드로 연결</b>에 넣기만 하면 됩니다. API 키는 서버 밖으로 나가지 않으며, 모든 사용 요금은 서버에 등록된 키(관리자)에게 청구됩니다.</p>`);
  const reopen = () => openDistAdmin(hash);
  body.querySelector('#da-new').addEventListener('click', async () => {
    const name = prompt('누구에게 드리는 코드인가요? (이름 또는 메모)');
    if (name === null) return;
    try {
      const r = await adminCall(hash, { action: 'create', name });
      await navigator.clipboard.writeText(r.code).catch(() => {});
      toast('발급 완료: ' + r.code + ' (복사됨)');
      reopen();
    } catch (e) { toast(e.message, 5000); }
  });
  body.querySelectorAll('[data-da-copy]').forEach(b => b.addEventListener('click', () => {
    navigator.clipboard.writeText(b.dataset.daCopy); toast('복사했습니다: ' + b.dataset.daCopy);
  }));
  body.querySelectorAll('[data-da-toggle]').forEach(b => b.addEventListener('click', async () => {
    try { await adminCall(hash, { action: 'toggle', code: b.dataset.daToggle, active: b.dataset.active !== '1' }); reopen(); }
    catch (e) { toast(e.message, 5000); }
  }));
  body.querySelectorAll('[data-da-limit]').forEach(b => b.addEventListener('click', async () => {
    const v = prompt('월 한도 (가중 토큰 수 — 예: 3000000 ≈ 설교 40편)', '3000000');
    if (v === null) return;
    try { await adminCall(hash, { action: 'limit', code: b.dataset.daLimit, limitTokens: +v }); reopen(); }
    catch (e) { toast(e.message, 5000); }
  }));
  body.querySelectorAll('[data-da-del]').forEach(b => b.addEventListener('click', async () => {
    if (!confirm(b.dataset.daDel + ' 코드를 삭제할까요? 그 사람은 더 이상 연결할 수 없습니다.')) return;
    try { await adminCall(hash, { action: 'delete', code: b.dataset.daDel }); reopen(); }
    catch (e) { toast(e.message, 5000); }
  }));
}
