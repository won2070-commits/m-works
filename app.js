...
function normProject(p) {
  ...
  p.passage = Object.assign({ ref: '', text: '', confirmed: false, candidates: [], check: null, genre: '' }, p.passage || {});
...
const LOGO_SVG = `<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" aria-label="M-Works 로고">
  ...
  <text x="50" y="83" font-family="Menlo, 'SF Mono', monospace" font-size="10" letter-spacing="3" text-anchor="middle" fill="#000000" opacity=".7">WORKS</text>
</svg>`;
...
function renderStep1(m, p) {
  ...
  const recommendCard = !p.passage.confirmed ? `
    <div class="card">
      ...
      <div class="btn-row">
        ...
        ${p.passage.candidates.length ? `<button class="btn btn-ghost" id="s1-more">본문 2~3개 더 찾기</button>` : ''}
  ...
  const more = $('#s1-more');
  if (more) more.addEventListener('click', () => recommend(p, 3, true));
  bindCands(p);
}
