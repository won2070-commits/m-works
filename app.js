...
function renderStep5(m, p) {
  ...
  m.innerHTML = `
    ...
    ${renderSongCard(p)}
    <div class="card">
      <h3>⏱ 시간·분량 조정 <span class="opt" style="font-weight:400;font-size:.78rem">— 어느 대목을 늘리고 줄일지, 그 자리에서 바로 누르세요</span></h3>
      ${(() => {
        const total = readingMinutes(htmlToText(p.draft.html));
        const diff = total - p.inputs.targetMin;
        const div = document.createElement('div'); div.innerHTML = p.draft.html;
        const heads = [...div.querySelectorAll('h2')].map(h => h.textContent.trim()).filter(t => t && !/추천 도서/.test(t));
        return `
        <p style="font-size:.9rem;margin-bottom:10px">현재 예상 <b>${fmtMin(total)}</b> / 목표 <b>${p.inputs.targetMin}분</b> —
          <span style="color:${Math.abs(diff) < 1.5 ? 'var(--success)' : 'var(--red)'};font-weight:700">${Math.abs(diff) < 1.5 ? '목표에 맞음 👍' : (diff > 0 ? fmtMin(diff) + ' 초과 → 줄일 대목을 고르세요' : fmtMin(-diff) + ' 여유 → 늘릴 대목을 고르세요')}</span></p>
        ${heads.length ? heads.map(h => `
          <div style="display:flex;align-items:center;gap:8px;padding:6px 4px;border-bottom:1px solid var(--hairline-soft)">
            <b style="flex:1;font-size:.9rem">${esc(h)}</b>
            <button class="btn btn-ghost btn-sm" data-rs-grow="${esc(h)}" ${aiConnected() ? '' : 'disabled'}>＋ 늘리기 ${AI_ICO}</button>
            <button class="btn btn-ghost btn-sm" data-rs-cut="${esc(h)}" ${aiConnected() ? '' : 'disabled'}>－ 줄이기 ${AI_ICO}</button>
          </div>`).join('') : '<p style="font-size:.84rem;opacity:.7">원고에 소제목(서론·본론·결론 등)이 있어야 대목별 조정이 가능합니다.</p>'}
      `; })()}
    </div>
    <div class="card">
  ...
  $$('#main [data-rs-grow]').forEach(b => b.addEventListener('click', () => resizeSection(p, b.dataset.rsGrow, +1)));
  $$('#main [data-rs-cut]').forEach(b => b.addEventListener('click', () => resizeSection(p, b.dataset.rsCut, -1)));
