...
/* ═══════════════════ 브랜드 ═══════════════════ */
const APP_VERSION = 'v12 · 2026-07-17';
...
function openManual() {
  modal('사용 설명서 — M.Works ' + APP_VERSION, `
    ...
    <h4 style="margin-top:0">1. 다섯 단계 흐름</h4>
    <p>왼쪽의 <b>새 설교 시작</b>을 눌러 기본 정보(주제·대상·목적·날짜)를 적고 아래 순서로 진행합니다.</p>
    <p>① <b>본문 찾기</b> — 주제를 넣으면 AI가 본문 5곳을 추천합니다. 마음에 없으면 요청사항을 적고 추가 추천을 받으세요. 확정하면 본문 전문을 성경에서 복사해 붙여넣습니다.<br>
    ② <b>중심사상</b> — 본문을 로빈슨 12단계로 분석해 한 문장 중심사상을 세웁니다.<br>
    ③ <b>설교 작성</b> — [🤖 설교문 작성]을 누르면 나의 작성 규칙·자료 서랍을 반영해 초안을 씁니다. 초안이 생기면 아래 <b>부분 재작성 메뉴</b>(제목 다시, 예화 추가, 추천 도서 추가 등)가 열립니다.<br>
    ④ <b>형식 결정</b> — 연역·귀납·4페이지 등 9가지 형식으로 바꿔보고 마음에 들면 승인합니다.<br>
    ⑤ <b>연습하기</b> — 피드백·제스처·쉼멈춤 추천을 받고, <b>📄 원고에서 위치 보기</b>로 본문 속 표시(🖐①·∕·⏸)를 확인합니다. 표시를 누르면 옆에 설명이 나옵니다. 리허설 모드로 시간을 재세요.</p>
    <h4>2. AI 연결</h4>
    <p>설정에서 Anthropic API 키를 넣거나, 이 컴퓨터에서 claude 로그인이 되어 있으면 자동 연결됩니다. 왼쪽 아래 배지에서 연결 상태를 확인하세요. 설교문 생성은 처음 1~3분간 조용히 구상한 뒤 글이 흐르기 시작합니다.</p>
    <h4>3. 자료 서랍</h4>
    <p>왼쪽 메뉴 <b>자료 서랍</b>에 예화·통계·간증·인용을 넣어 두면(파일을 끌어다 놓아도 됨) 설교문 작성 때 자동으로 곳곳에 인용됩니다. 3단계의 자료 투입 카드로 이미 쓴 원고에 녹일 수도 있습니다.</p>
    <h4>4. 내보내기·인쇄</h4>
    <p>왼쪽 <b>내보내기</b>에서 Word(.doc)·텍스트·인쇄를 선택합니다. 체크박스로 <b>쉼·멈춤 표시(∕·⏸)</b>와 <b>제스처 위치 표시(🖐①)</b>의 포함 여부를 정할 수 있습니다. Word 양식은 나의 작성 규칙의 원고 양식(맑은 고딕 10pt, 한 문장 한 줄)을 따릅니다.</p>
    <h4>5. 저장·백업</h4>
    <p>모든 데이터는 이 기기 브라우저에 자동 저장됩니다(서버 전송 없음). 보관함의 <b>전체 백업(JSON)</b>으로 파일 백업을 권장합니다. 기기를 바꿀 때 이 파일을 가져오기 하면 됩니다.</p>
    <h4>6. 업데이트·버전</h4>
    <p>왼쪽 아래에 현재 버전(예: ${APP_VERSION})이 표시됩니다. 새 버전이 배포되면 앱을 열 때 자동으로 확인해 한 번 새로고침하며 적용됩니다.</p>
    <h4>7. 비밀번호 잠금</h4>
    <p>설정 → <b>🔒 비밀번호 걸기</b>로 앱을 열 때 비밀번호를 묻게 할 수 있습니다. 이 기기에만 저장되므로 잊지 않게 메모해 두세요. <b>나가기</b>를 누르면 다음에 열 때 다시 묻습니다.</p>
    </div>`);
}
  ...
  m.innerHTML = `
    <div style="text-align:center; padding:40px 0 30px">
      <div style="display:flex;justify-content:center;margin-bottom:10px">
        <svg viewBox="0 0 188 100" xmlns="http://www.w3.org/2000/svg" aria-label="M.Works 로고" style="height:88px">
        <rect x="4" y="4" width="72" height="72" rx="18" fill="#FFC000"/>
        <text x="40" y="52" transform="scale(1 1.2)" font-family="-apple-system, BlinkMacSystemFont, 'Helvetica Neue', Arial, sans-serif" font-size="44" font-weight="800" text-anchor="middle" fill="#ffffff">M</text>
        <rect x="48" y="42" width="44" height="44" rx="12" fill="#FF5600" stroke="#ffffff" stroke-width="3"/>
        <text x="70" y="62.7" transform="scale(1 1.2)" font-family="-apple-system, BlinkMacSystemFont, 'Helvetica Neue', Arial, sans-serif" font-size="26" font-weight="800" text-anchor="middle" fill="#000000">W</text>
        <text x="97" y="62.7" transform="scale(1 1.2)" font-family="-apple-system, BlinkMacSystemFont, 'Helvetica Neue', Arial, sans-serif" font-size="31" font-weight="800" text-anchor="start" fill="#000000">orks</text>
      </svg>
      </div>
      <p style="color:var(--ink-soft);margin-top:6px">설교자를 위한 설교 작성 5단계</p>
      <div class="btn-row" style="justify-content:center;margin-top:44px;margin-bottom:56px">
...
function renderClinic(m) {
  m.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="chip-row">
        ...
        <label class="chip" style="cursor:pointer">📄 텍스트 파일<input id="cl-file" type="file" accept=".txt,.md,.text" style="display:none"></label>
  ...
  $('#cl-file').addEventListener('change', e => {
    const f = e.target.files[0]; if (!f) return;
    const fr = new FileReader(); fr.onload = () => { $('#cl-text').value = fr.result; setStatus('파일을 읽었습니다: ' + f.name); }; fr.readAsText(f);
  });
...
function renderPromptLibrary(m) {
  ...
  m.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="field"><label>사용자 프롬프트 틀 <span class="opt">({{슬롯}}은 앱이 채우는 자리 — 지우지 마세요)</span></label><textarea id="pl-user" style="min-height:180px;font-family:var(--mono);font-size:.74rem"></textarea></div>
      <div class="btn-row">
...
function openImport() {
  ...
  body.querySelector('#im-file').addEventListener('change', e => {
    const f = e.target.files[0]; if (!f) return;
    const fr = new FileReader();
    fr.onload = () => { gotText = fr.result; $('#im-paste').value = gotText; setStatus('파일을 읽었습니다: ' + f.name); };
    fr.readAsText(f);
  });
  body.querySelector('#im-img').addEventListener('change', e => {
    const f = e.target.files[0]; if (!f) return;
    setStatus('사진에서 원고를 인식하는 중… (1~3분)', true);
    const fr = new FileReader();
    fr.onload = async () => {
      try {
        ...
        const r = await fetch('/api/ocr', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ data: b64, mime: f.type, model: DB.settings.model }) });
        const j = await r.json();
        if (j.error) { setStatus('⚠ ' + j.error); return; }
        gotText = j.text; $('#im-paste').value = gotText; setStatus('원고를 인식했습니다. 내용을 확인해 주세요.');
      } catch (err) { setStatus('⚠ 인식 실패: ' + err.message); }
    };
    fr.readAsDataURL(f);
  });
...
function openPromptManager() {
  ...
  const body = modal('프롬프트 관리', `
    ...
    <div class="field"><label>시스템 지시</label><textarea id="pm-system" style="min-height:120px;font-family:monospace;font-size:.78rem"></textarea></div>
    <div class="field"><label>사용자 프롬프트 틀</label><textarea id="pm-user" style="min-height:200px;font-family:monospace;font-size:.78rem"></textarea></div>
    <div class="btn-row">
