...
/* ═══════════════════ 브랜드 ═══════════════════ */
const APP_VERSION = 'v45 · 2026-07-21';
...
function renderSongCard(p) {
  ...
  return `
    <div class="card">
      ...
      <p class="ai-note" style="margin-top:10px">악보와 음원은 저작권이 있어 앱 안에 담지 않고, 누르면 검색 결과로 연결합니다. 예배에 사용할 때는 저작권(CCLI 등) 규정을 확인해 주세요.</p>
    </div>`;
}
function bindSongCard(p) {
  ...
  $$('#main [data-song]').forEach(b => b.addEventListener('click', () => {
    ...
    const body = modal('🎵 ' + (s.num ? s.num + '장 ' : '') + s.title, `
      ...
      <p style="font-size:.82rem;color:var(--ink-soft);margin:12px 0 8px">아래를 누르면 새 창에서 열립니다.</p>
      <div class="chip-row">
        <a class="chip song-link" href="${u.score}" target="_blank" rel="noopener">🎼 악보 찾기</a>
      ...
      <div class="btn-row"><button class="btn btn-ghost btn-sm" id="song-copy">📋 곡 이름 복사</button></div>`);
