...
const APP_VERSION = 'v68 · 2026-07-22';
...
function openBlockPicker(p, title, srcHtml) {
  ...
  ['mouseup', 'keyup', 'touchend'].forEach(ev => src.addEventListener(ev, () => setTimeout(readSel, 10)));
  useBtn.addEventListener('click', () => { if (picked) openPlacePicker(p, picked); });
  body.querySelector('#bp-all').addEventListener('click', () => openPlacePicker(p, htmlToText(srcHtml).trim()));
  body.querySelector('#bp-cancel').addEventListener('click', closeModal);
}
/* 2) 원고 전체를 띄우고, 문단을 찍으면 그 자리에 넣는다 */
function openPlacePicker(p, blockText) {
  syncEditor();
  ...
  const body = modal('어디에 넣을까요 — 원고에서 자리를 찍으세요', `
    ...
    <div class="fb-item" style="margin-top:10px;max-height:96px;overflow:auto"><b>넣을 블럭 · ${blockText.length.toLocaleString()}자</b>${esc(blockText.slice(0, 280))}${blockText.length > 280 ? '…' : ''}</div>
    <div class="btn-row">
...
async function openBookFlow(p) {
  ...
  openBlockPicker(p, '📚 추천 도서 2권', srcHtml);
}
